...
/-!
# Varying Shell Density

Natural-density transport for shell-dependent good sets.

The final bootstrap chooses a different tolerance and a different number of
stages on each dyadic shell.  A fixed `(C,D)` theorem therefore cannot be
applied globally.  This file proves the required varying-rate statement:
if the exceptional proportion on shell `M` tends to zero, then assembling
the shellwise good sets gives a natural-density-one set.
-/
...
/-- Natural density one, encoded by convergence of the exceptional prefix
proportion to zero. -/
def HasNaturalDensityOne (S : Set ℕ) : Prop :=
  Filter.Tendsto
    (fun N : ℕ => ((badPrefix S N).card : ℝ) / N)
    Filter.atTop (nhds 0)

/-- Assemble one good set per dyadic shell. -/
...
/-- Exceptional proportion on one dyadic shell. -/
...
/-- A `(C,D)` prefix-density certificate controls the exceptional
proportion of its `M`-th dyadic shell. -/
theorem shellExceptionalRatio_le_of_isCDDense
    {S : Set ℕ} {C D : ℝ} (hS : IsCDDense S C D) (M : ℕ) :
    shellExceptionalRatio S M ≤
      2 * C * Real.exp (-(Real.log 2 * D * M)) := by
  have hNpos : 0 < 2 ^ (M + 1) := by positivity
  have hsub :
      shellBad S M ⊆ badPrefix S (2 ^ (M + 1)) := by
    intro n hn
    simp only [shellBad, Finset.mem_filter, dyadicShell, Finset.mem_Ico,
      badPrefix, positivePrefix, Finset.mem_Icc] at hn ⊢
    exact ⟨⟨by omega, hn.1.2.le⟩, hn.2⟩
  have hcard :
      ((shellBad S M).card : ℝ) ≤
        ((badPrefix S (2 ^ (M + 1))).card : ℝ) := by
    exact_mod_cast Finset.card_le_card hsub
  have hbad := hS.bad_bound (2 ^ (M + 1)) hNpos
  have hraw :
      shellExceptionalRatio S M ≤
...
/-- **Varying-rate dyadic-shell lemma.** -/
theorem hasNaturalDensityOne_assembleDyadic
    (S : ℕ → Set ℕ)
    (hvanish :
      Filter.Tendsto
        (fun M => shellExceptionalRatio (S M) M)
        Filter.atTop (nhds 0)) :
    HasNaturalDensityOne (assembleDyadic S) := by
  classical
  rw [HasNaturalDensityOne, Metric.tendsto_atTop]
  intro eps heps
  obtain ⟨M₀, hM₀⟩ :=
          ...
          ≤ ∑ M ∈ Finset.range M₀, (2 : ℝ) ^ M := by
            ...
            have hcard :
              ...
              calc
                ...
                _ = 2 ^ M := by simp [dyadicShell, Nat.card_Ico, pow_succ]
                    <;> omega
  ...
  have hearlySmall : (2 : ℝ) ^ M₀ < eps * N / 8 := by
    ...
    nlinarith
  have hratio :
      ((badPrefix (assembleDyadic S) N).card : ℝ) / N < eps := by
    have hNR0 : (0 : ℝ) < N := by exact_mod_cast hNpos
    rw [div_lt_iff₀ hNR0]
    nlinarith
  have hratio0 :
      0 ≤ ((badPrefix (assembleDyadic S) N).card : ℝ) / N := by
    positivity
