...
/-!
# Stretched Exceptional Count

Dyadic summation for stretched-exponential shell bounds.

If the exceptional proportion on shell `M` is at most

  A * exp (-c * (M + 4)^alpha),

with `0 < alpha <= 1`, then the exceptional count up to `N` has the same
stretched-exponential power.  This is purely a shell-summation theorem; it
contains no Collatz input.
-/
...
/-- A conservative rate that simultaneously pays for the late-shell
majorant and the early-shell cutoff. -/
...
`0 < alpha <= 1`. -/
theorem card_badPrefix_assembleDyadic_le_stretched_rate
...
theorem card_badPrefix_assembleDyadic_le_stretched_log
    (S : ℕ → Set ℕ) (M₀ N : ℕ) (A c alpha : ℝ)
    (hA : 0 ≤ A) (hc : 0 < c)
    (halpha0 : 0 < alpha) (halpha1 : alpha ≤ 1)
    (hshell :
  ...
  let L := Nat.log 2 N
  have hbase :=
    card_badPrefix_assembleDyadic_le_stretched_rate
